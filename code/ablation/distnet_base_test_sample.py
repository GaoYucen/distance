...
def test_model(eval_loader, config, maxLength):
    ...
    # Load pre-trained distnet_base model
    if config.type == 1:
        model.load_state_dict(torch.load("param/distnet_base_best_chengdu_1.ckpt", map_location=device))
    elif config.type == 2:
        model.load_state_dict(torch.load("param/distnet_base_best_chengdu_tilde_L1.ckpt", map_location=device))
    elif config.type == 3:
        model.load_state_dict(torch.load("param/distnet_base_best_chengdu_L1.ckpt", map_location=device))
    ...
    if config.type == 1:
        ...
        result_file = "log/distnet_base_L1_results_sample.txt"
