...
def try_load_embedding(data_dir: Path, orig_ids: list) -> np.ndarray | None:
    ...
    emb = None
    # search for *.emb
    emb_files = list(data_dir.glob("*node2vec*.emb")) + list(data_dir.glob("*node*vec*.emb"))
...
def process_city(root: Path, city_name: str, data_dir: Path, 
    ...
    if sdm_file.exists():
        ...
        if hasattr(sdm, 'toarray'):
            ...
            try:
                ...
                # Note: toarray() sets unset values to 0, but we need inf for unreachable pairs
                # We stored all finite values, so unset positions should be inf
                sdm_dense = sdm.toarray()
                # Replace 0 values that weren't explicitly stored with inf
                # But we need to be careful: some 0 values might be real distances
                # Solution: use a very large sentinel value during computation, or
                # better: set all positions not in the sparse matrix to inf
                # For now, we'll set all zeros (except possibly diagonal) to inf
                # This is a heuristic - ideally we'd track which zeros are real
                ...
                logging.error("Consider using a machine with more RAM or processing in chunks")
                raise
    ...
    np.save(out_pre / "preprocessed_indices.npy", inds.astype(int))
...
def main():
    ...
    p.add_argument("--data-dir", default="/home/lizhuoran/distance/data", help="Path to data folder containing city subfolders or city files")
