...
济南：8,908 个原始节点，23,312 条原始弧；保留 8,840 个节点、23,206 条有向弧。
深圳：11,933 个原始节点，27,410 条原始弧；保留 11,738 个节点、27,105 条有向弧。
...
|---|---:|---:|
| 可辨识测试 OD / 总测试 OD | 9,988 / 10,000 | 4,993 / 5,000 |
| alpha ≥ 20% 的比例 | 11.924% | 7.611% |
| 解释的方向差平方能量 | 63.842% | 31.248% |
| 短距离切片的方向差平方能量解释率 | 16.800% | -10.188% |
| 真实 S 的中点参考 MRE | 5.202571% | 3.910935% |
| 真实 S + 训练拟合势差的 MRE | 4.850292% | 4.141037% |
...
|---|---:|---:|
| L1 | 9.804185 ± 0.093491 | 6.912566 ± 0.089693 |
| tilde-L1（63+1） | 9.765522 ± 0.055684 | 6.942988 ± 0.087867 |
| tilde-L1（62+2） | 9.736191 ± 0.084128 | 6.917038 ± 0.122065 |
| 对称 L-infinity | 10.109872 ± 0.141080 | 7.168693 ± 0.145067 |
| 非对称 tilde-L-infinity | 10.563762 ± 0.125222 | 7.670803 ± 0.117943 |

± 是三个种子之间的样本标准差，不是置信区间。模型包含原始负预测率和高非对称切片结果，详见 pilot_summary.json。
30 次运行中，有 14 次最佳验证轮次落在最后 5 轮；这个数字只是训练预算边界的提醒，不单凭它宣称收敛或未收敛。
...
固定使用与神经模型完全相同的 20,000 个训练 OD 组拟合自由节点势函数。在 4,791/5,000 个势差可辨识的测试组上，与已保存的模型预测比较。没有重新训练神经模型。
自由节点势函数是训练最小二乘参考，不是测试最优上界，也不是完整距离索引；不同模型都在相同可辨识测试子集上计算下表。

| 方法 | 测试方向差平方能量解释比例 |
|---|---:|
| 20,000 训练组拟合的自由节点势函数 | 42.328% |
| l1（三种子均值） | 0.000% |
| tilde_63_1（三种子均值） | 2.818% |
| tilde_62_2（三种子均值） | 5.939% |
| linf_symmetric（三种子均值） | 0.000% |
| linf_asymmetric（三种子均值） | 11.861% |

该对照用于区分自由节点势差参考与当前编码器/训练所得方向表示；它不单独证明差距完全来自优化，因为两者参数化与训练目标不同。原始对称/方向平方误差分解保存在 matched_budget_decomposition.json。
